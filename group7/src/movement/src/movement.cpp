...
void Mover::driveForwardOdom(double distance)
{
  ...
  while (((ros::Time::now()-startTime).toSec()*0.4 < distance) && node.ok() && keepMoving)
    {
...
{
  srand (time(NULL));
  double randDist = 2*((double) rand() / (RAND_MAX)) + 0.25;
  double randAngle = 180*((double) rand() / (RAND_MAX));
