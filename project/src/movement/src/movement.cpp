...
//angle in rad
void Mover::rotateTurtle(double angle)
{
    double angVel = 0.25;
    double target_time = angle/angVel;
    geometry_msgs::Twist base_cmd;
    base_cmd.linear.y = 0;
    base_cmd.linear.x = 0;
    base_cmd.angular.x = base_cmd.angular.y = 0;
    ros::Rate rate(100);
    while( ros::Time::now().toSec() <= (ros::Time::now().toSec() + target_time)  )
    {
        base_cmd.angular.z = angVel;
...
void Mover::scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    m_distanceToMarker = scan->ranges[(maxIndex-minIndex)/2];
...
void Mover::moveRandomly()
{
    ...
    rotateTurtle(randAngle * M_PI/180);
